...
def main():
    ...
    while True:
        ...
        lmList = detector.getPosition(img)  # Get landmarks

        if lmList:
            print("Landmarks:", lmList)  # Debug print of the landmarks

        # Display the FPS on the image
        cTime = time.time()
        fps = 1 / (cTime - pTime)
        pTime = cTime
        cv2.putText(img, f'FPS: {int(fps)}', (50, 50), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
